...
class RPS:
    """Initialize variables of our Readme Python Game Class"""

    def __init__(self, token, issueNumber, repo) -> None:
        self.token = token
        ...
        if(len(move) > 1) and move[1] in self.moves:
            ...
            if move in self.moves:
                ...
                if result = True:
                    ...
                    self.writeToRepo(
                        self.filePath, f"@{userName} won with action {action}", newFileData, fileData.sha)
                elif result == None:
                    self.addComment('Oops! This was a draw! :eyes:')
                    self.writeToRepo(
                        self.filePath, f"@{userName} played {action}", newFileData, fileData.sha)
                elif result == False:
                    action = self.getEmoji(self.computerMove)
                    self.addComment(
                        f'Uh-Oh! You lost! :eyes:\n Computer played {self.computerMove}')
                    self.writeToRepo(
                        self.filePath, f":robot: won with {action}", newFileData, fileData.sha)
        else:
    ...
    def genFileData(self, userName, result):
        ...
        with outer:
            ...
            h3(
                a(":punch:",
                  href=f'https://github.com/{repo}/issues/new?title=rps|rock'),
                a(":hand:",
                  href=f'https://github.com/{repo}/issues/new?title=rps|paper'),
                a(":scissors:",
                  href=f'https://github.com/{repo}/issues/new?title=rps|scissors'))
            if result == True:
